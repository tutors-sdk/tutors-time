...
const currentRange: string = new Date().getFullYear().toString(); // Initially set to a default year
...
export class CalendarChart {
  ...
  renderChart(timeActiveMap: Map<string, Map<string, number>>, session: Session) {
    const chartContainer = this.getChartContainer(tutorsConnectService.tutorsId.value.login);

    if (!chartContainer) {
      console.error("Chart container not found for user:", tutorsConnectService.tutorsId.value.login);
      return;
    }

    const callendarMapCollection: CalendarMap[] = [];
    for (const [key, value] of timeActiveMap.entries()) {
      if (key.includes(tutorsConnectService.tutorsId.value.login)) {
    ...
    if (!sessionStorage.getItem("logoShown")) {
      ...
      this.myCharts[tutorsConnectService.tutorsId.value.login] = chart;
      const option = calendar(session, callendarMapCollection, bgPatternImg, currentRange);

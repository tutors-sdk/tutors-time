...
export class BaseHeatMapChart<T> {
  ...
  async populatePerUserSeriesData(
    allItems: Lo[],
    userId: string,
    index: number,
    learninObjValue: string
  ): Promise<number[][]> {
  ...
  async populateAndRenderSingleUserData(session: Session, allItems: Lo[], learninObjValue: string) {
    ...
    const userId = tutorsConnectService.tutorsId.value.login;
    this.yAxisData = [userId];

    const seriesData: number[][] = await this.populatePerUserSeriesData(
      allItems,
      tutorsConnectService.tutorsId.value.login,
      0,
      learninObjValue
    );
    this.series = {

...
export function piechart(
  ...
  singleUserInnerData: DrilledDownData[],
  singleUserOuterData: DrilledDownData[]
) {
  return {
    tooltip: {
      trigger: "item",
    ...
    series: [
      {
        ...
        data:
          allUsersTopicActivity.length !== 0
            ? allUsersTopicActivity.filter((topic) => topic.value > 0)
            : singleUserInnerData.filter((topic) => topic.value > 0) || []
      },
      {
        name: "Outer Pie",
        type: "pie",

...
export function calendar(
  ...
  return {
    title: {
      top: 30,
      left: "center",
      text: tutorsConnectService.tutorsId.value.login,
      link: "https://www.github.com/" + tutorsConnectService.tutorsId.value.login,
      target: "self"
    },
    backgroundColor: {
      image: bgPatternImg,
    ...
    graphic: [
      {
        ...
        bounding: "raw",
        style: {
          image: tutorsConnectService.tutorsId.value.login, // URL to user's profile picture
          width: 50,
          height: 50
        }
      }
...
export function calendarCombined(
  ...
  return {
    ...
    series: {
      ...
      coordinateSystem: "calendar",
      data:
        Array.from(calendarMap)?.map(([date, timeActive]) => [
